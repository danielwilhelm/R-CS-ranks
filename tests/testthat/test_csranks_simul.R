...
test_that("return value is of correct class and size", {
  for (cstype in c("two-sided", "lower", "upper")) {
    for (stepdown in c(TRUE, FALSE)) {
      res1 <- csranks_simul(1:10, V, coverage = 0.95, cstype = cstype, stepdown = stepdown, R = 100, indices = NA)
      res2 <- csranks_simul(1:10, V, coverage = 0.95, cstype = cstype, stepdown = stepdown, R = 100, indices = 1)
      res3 <- csranks_simul(1:10, V, coverage = 0.95, cstype = cstype, stepdown = stepdown, R = 100, indices = 1:3)
