...
test_that("count_lequal_lesser works for sorted x", {
  ...
  expected <- list(
    n_lequal = c(0, 1, 1, 2, 5, 5, 7, 7, 8, 8),
...
test_that("irank's increasing argument works", {
  ...
  expect_equal(
    irank(x, increasing = TRUE, omega = 0.4),
    expected_irank_om0.4
  )
  expect_equal(
    irank(x_neg, increasing = FALSE, omega = 0.4),
    expected_irank_om0.4
  )
})
...
test_that("irank_against's increasing argument works", {
  ...
  expect_equal(
    irank_against(x, v, omega = 0.5, increasing = TRUE),
    expected_irank
  )
})
...
test_that("irank_against handles NAs", {
  ...
  expect_equal(
    irank_against(x, v, omega = 0.5, na.rm = FALSE),
    expected_irank
  )
  expect_equal(
    irank_against(x, v, omega = 0.5, na.rm = TRUE),
    expected_irank[-4]
  )
})
