...
test_that("`x` argument is handled correctly by process_csranks_args", {
  expect_equal(
    process_csranks_args(1:10, V, NA, na.rm = TRUE),
...
test_that("`Sigma` argument is handled correctly by process_csranks_args", {
  ...
  expect_error(
    process_csranks_args(1:10, 1:10 / 10, NA, na.rm = TRUE),
    "variances"
  )
...
test_that("NAs are handled correctly by process_csranks_args", {
  ...
  expect_equal(
    process_csranks_args(c(1:8, NA, 10),
      V, NA,
      na.rm = TRUE
    ),
    expected_args
...
test_that("adjust_indices_for_NAs work correctly", {
  expect_equal(
    adjust_indices_for_NAs(1:10, rep(TRUE, 10)),
    1:10
  )
  expect_equal(
    adjust_indices_for_NAs(1:10, c(
      rep(TRUE, 4),
      rep(FALSE, 3),
      rep(TRUE, 3)
    )),
  ...
  expect_equal(
    adjust_indices_for_NAs(seq(2, 10, 2), c(
...
test_that("`x` argument is handled correctly by process_csranks_multinom_args", {
  expect_equal(
    process_csranks_multinom_args(1:10, NA, na.rm = TRUE)[["x"]],
    1:10
  )
  expect_equal(
    process_csranks_multinom_args(c(NA, 2:10), NA, na.rm = TRUE)[["x"]],
    2:10
  )
...
test_that("`indices` argument is handled correctly by process_csranks_multinom_args", {
  expect_equal(
    process_csranks_multinom_args(1:10, NA, na.rm = TRUE)[["indices"]],
    1:10
  )
  expect_equal(
    process_csranks_multinom_args(c(NA, 2:10), NA, na.rm = TRUE)[["indices"]],
    1:9
  )
  expect_equal(
    process_csranks_multinom_args(c(NA, 2:10), 1:10, na.rm = TRUE)[["indices"]],
    1:9
  )
  expect_equal(
    process_csranks_multinom_args(c(NA, 2:10), 2:10, na.rm = TRUE)[["indices"]],
    1:9
  )
  expect_equal(
    process_csranks_multinom_args(c(NA, 2, 3, NA, NA, 6:10),
      c(1, 3, 5, 6),
      na.rm = TRUE
    )[["indices"]],
    c(2, 3)
...
test_that("`indices` argument is handled correctly by process_indices_argument", {
  expect_equal(
    process_indices_argument(1:10, 10),
    1:10
  )
  expect_equal(
    process_indices_argument(c(NA, 2:10), 10),
    1:10
  )
  expect_equal(
    process_indices_argument(c(-2, -5, -6), 10),
    c(1, 3, 4, 7, 8, 9, 10)
  )
