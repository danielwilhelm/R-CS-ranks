...
test_that("Initialize_I0 works for patrial indices", {
  expect_equal(
    initialize_I0(p, indices, TRUE, "upper"),
    list(
      I0 = expected_partial_I0,
      cstype = "upper"
    )
  )
  expect_equal(
    initialize_I0(p, indices, FALSE, "symmetric"),
    list(
      I0 = expected_partial_I0,
      cstype = "symmetric"
    )
  ...
  expect_equal(
    initialize_I0(p, indices, TRUE, "symmetric"),
    list(
      I0 = expected_symmetric_I0,
      cstype = "lower"
    )
  )
})
...
expected_requested_differences <- matrix(
  c(
    2, 1,
    1, 2,
    3, 2,
...
expected_reduced_I <- list(
  needed_variables = expected_needed_variables,
...
test_that("calculate_scaled_diffs works as master 0.2.2", {
  ...
  Zdiff_scaled <- calculate_scaled_differences_in_samples(
    Z, requested_differences,
    sigmadiff[I]
  )
  expect_equal(Zdiff_scaled[1, ], expected_scaled_diff)
  expect_equal(
    apply(Zdiff_scaled, 1, max),
