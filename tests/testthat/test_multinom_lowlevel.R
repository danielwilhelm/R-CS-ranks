...
expected_symmetric_pairs <- matrix(
  c(
...
test_that("which_pairs_are_compared works", {
  expect_equal(
    which_pairs_are_compared(p, indices, "upper"),
    expected_partial_pairs
  )
  expect_equal(
    which_pairs_are_compared(p, indices, "two-sided"),
    expected_symmetric_pairs
  )
  expect_equal(
    which_pairs_are_compared(p, indices, "lower"),
...
test_that("reject_or_accept_works", {
  expect_equal(
    reject_or_accept(df, "Bonferroni", coverage),
    expected_df_B
  )
  expect_equal(
    reject_or_accept(unordered_df, "Bonferroni", coverage),
    expected_df_B
  )
  expect_equal(
    reject_or_accept(df, "Holm", coverage),
    expected_df_H
  )
  expect_equal(
    reject_or_accept(unordered_df, "Holm", coverage),
    expected_df_H
  )
})
...
test_that("reject_or_accept works for Holm in edge cases", {
  expect_equal(
    reject_or_accept(df_edge_H, "Holm", coverage),
    expected_df_edge_H
  )
  expect_equal(
    reject_or_accept(df_edge_2_H, "Holm", coverage),
...
test_that("calculate_N_plus_minus works", {
  expect_equal(
    calculate_N_plus_minus(df, cstype = "two-sided", indices = 1:4),
    expected_symmetic
  )
  expect_equal(
    calculate_N_plus_minus(unordered_df, cstype = "two-sided", indices = 1:4),
    expected_symmetic
  )
  expect_equal(
    calculate_N_plus_minus(df, cstype = "upper", indices = 1:4),
    expected_upper
  )
  expect_equal(
    calculate_N_plus_minus(unordered_df, cstype = "upper", indices = 1:4),
    expected_upper
  )
  expect_equal(
    calculate_N_plus_minus(df, cstype = "lower", indices = 1:4),
    expected_lower
  )
  expect_equal(
    calculate_N_plus_minus(unordered_df, cstype = "lower", indices = 1:4),
    expected_lower
  )
})
...
test_that("calculate_N_plus_minus works with indices argument", {
  expect_equal(
    calculate_N_plus_minus(df, cstype = "two-sided", indices = indices),
    expected_symmetic
  )
  expect_equal(
    calculate_N_plus_minus(df, cstype = "upper", indices = indices),
    expected_upper
  )
  expect_equal(
    calculate_N_plus_minus(df, cstype = "lower", indices = indices),
    expected_lower
  )
})
