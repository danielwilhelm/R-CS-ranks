#' @export
calculate_grouped_lmranks_variances <- function(object, complete = TRUE, ...){
  global_RX <- attr(object, "RX")
  global_RY <- attr(object, "RY")
  
  variances <- sapply(1:length(object), function(j){
    ...
    g_sum <- (h_1 + h_2 + h_3) / var_estimate
    g_sum <- matrix(g_sum, ncol = 1)
    sigmahat <- (t(g_sum) %*% g_sum) / (length(global_RX) ^ 2)
    # first division by n caused by E[h^2] estimate, second by CLT correction
    sigmahat
  })
  
  names(variances) <- names(object)
  return(variances)
}
